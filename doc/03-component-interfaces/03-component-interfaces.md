...
- **Name**: Kaiden Stewart
- **Dot Number**: steart.2292
- **Due Date**: 3/8 11:59 pm
...
- [x] I have created a kernel interface
  - [x] I have created a new Java file in `src`
  - [x] I have named the new Java file correctly (e.g., `NaturalNumberKernel`)
  - [x] I have included all of my kernel methods as method headers only
  - [x] I have written contracts for every single kernel method
    - [x] I have included an `@ensures` in every single kernel method
    - [x] I have included an `@requires` for every precondition, when applicable
    - [x] I have included an `@param` for every single parameter
    - [x] I have included an `@return` for every method with a return type
    - [x] I have included parameter modes where applicable
- [x] I have created an enhanced interface
  - [x] I have created a new Java file in `src`
  - [x] I have named the new Java file correctly (e.g., `NaturalNumber`)
  - [x] I have included all of my secondary methods as method headers only
  - [x] I have written contracts for every single secondary method
    - [x] I have included an `@ensures` in every single secondary method
    - [x] I have included an `@requires` for every precondition, when applicable
    - [x] I have included an `@param` for every single parameter
    - [x] I have included an `@return` for every method with a return type
    - [x] I have included parameter modes where applicable
...
- [x] I have converted my kernel interface to a PDF
- [x] I have converted my enhanced interface to a PDF
- [x] I am prepared to submit all three PDFs on Carmen
...
First I changed the following methods from kernel Methods to secondary methods:
- `String getTeamName(int index)`
- `String getTeamWins(int index)`
- `String getTeamLosses(int index)`
- `String getDivisionWins(int index)`
- `String getDivisionLosses(int index)`
- `String getTeamWinsLast10(int index)`
- `String getTeamLossesLast10(int index)`
- `String getPointDifferential(int index)`
- `String getStrengthRating(int index)`
- `String setStrengthRating(int index, double rating)`

This was done to limit the number of Kernel Methods needed. All of the previous methods will now use one of the following two methods:
- `String getStat(int teamIndex, int statIndex)`
- `String setStat(int teamIndex, int statIndex, String stat)`

I also decided to add two new instance variables:
- `private int this.lowestPointDifferential`
- `private int this.highestPointDifferential`

These instance variables will be used to store the lowest and highest point differential in the league so that point differential will can be used as another measure to compare a team's strength.
In order to access and set these instance methods the following kernel methods were created:
- `int getLowestPointDifferential()`
- `int getLowestPointDifferential()`
- `int setLowestPointDifferential(int pointDifferential)`
- `int setHighestPointDifferential(int pointDifferential)`

To find and set the lowest and highest point differntial the following secondary method was added:
- `void findAndSetLowestAndHighestPointDifferential()`

Another kernel method I added was:
- `void switchTeamIndexes(int indexTeamOne, int indexTeam 2)`

This method will be used to reorder the teams in the `void orderTeamsByStrengthRating()` secondary method.

Finally, I added the `void printToFile(String fileName)` to print the results to a file instead of my original plan to make the results as the `toString()` method as I think it would be useful for the `toString()` method to print the entire 2D array for debugging and/or being able to visualize the data.
